...
const CourseCard = ({
  ...
  return (
    <div className="w-full max-w-sm flex flex-col bg-neutral-950 border border-neutral-900 rounded-[24px] sm:rounded-[32px] overflow-hidden hover:border-red-600/30 transition-all duration-500 group relative">
      {/* Background Gradient Blob for ambience */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-red-600/10 blur-[80px] rounded-full pointer-events-none -z-0" />

      {/* --- SECTION 1: Top Area --- */}
      {/* Reduced padding for mobile (p-4) vs desktop (sm:p-6) */}
      <div className="p-4 sm:p-6 pb-2 flex justify-between items-start z-10 gap-4">
        {/* LEFT COL: Level & Requirements */}
        <div className="flex flex-col gap-3 max-w-[60%]">
          {/* Level Badge */}
          <div
            className={`
              ${glassStyle} px-3 py-1.5 sm:px-4 sm:py-2 rounded-full flex items-center gap-2 w-fit
            `}
          >
            <div
              className={`w-2 h-2 shrink-0 rounded-full ${
                isBeginner ? "bg-emerald-400" : "bg-purple-400"
              }`}
            />
          ...
          {/* Requirements Badge */}
          <div
            className={`${glassStyle} p-2 sm:p-3 rounded-2xl flex flex-col gap-1`}
          >
      ...
      {/* --- SECTION 2: Title & Stats --- */}
      <div className="px-4 sm:px-6 py-2 sm:py-4 z-10">
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 leading-tight group-hover:text-red-500 transition-colors">
          {title}
        </h3>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs sm:text-sm font-medium">{lectures}</span>
          </div>
          <div className="flex items-center gap-2">
            <BookOpen size={16} className="text-blue-500 shrink-0" />
            <span className="text-xs sm:text-sm font-medium">{practices}</span>
          </div>
        </div>
      </div>

      {/* --- SECTION 3: Pricing Blocks --- */}
      <div className="px-4 sm:px-6 py-2 grid grid-cols-2 gap-2 sm:gap-3 z-10">
        ...
        >
          ...
          {/* whitespace-nowrap prevents line break, text-lg scales down for mobile */}
